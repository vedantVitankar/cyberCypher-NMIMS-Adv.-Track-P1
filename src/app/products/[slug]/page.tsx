...
import { Star, ShoppingCart, Heart, ShieldCheck, Truck, RefreshCw, ChevronRight } from 'lucide-react';
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <nav className="mb-8 flex items-center gap-2 text-sm text-white/50">
        ...
        {product.category && (
          <>
            <Link href={`/products?category=${product.category.slug}`} className="hover:text-cosmic-orange transition-colors">
            ...
            <div className="flex items-center gap-4">
              <div className="flex items-center text-cosmic-orange">
                {Array.from({ length: 5 }).map((_, i) => (
                  <Star
                    key={i}
                    className={`h-4 w-4 ${i < Math.floor(product.rating) ? 'fill-current' : 'text-white/20'}`}
                  />
                ))}
                <span className="ml-2 text-sm font-semibold">{product.rating}</span>
              </div>
              <span className="text-white/40 text-sm">|</span>
              <span className="text-white/40 text-sm">{product.review_count} Reviews</span>
              <span className="text-white/40 text-sm">|</span>
              <span className={product.stock_quantity > 0 ? "text-green-500 text-sm font-medium" : "text-red-500 text-sm font-medium"}>
          ...
          <div className="mb-8 p-6 rounded-2xl bg-white/5 border border-white/10">
            <div className="flex items-end gap-3 mb-2">
              ...
              {product.compare_at_price && (
                <span className="text-xl text-white/40 line-through mb-1">${product.compare_at_price}</span>
              )}
      ...
      <div className="mb-24">
        <Tabs defaultValue="description" className="w-full">
          <TabsList className="bg-deep-charcoal border-white/10 w-full justify-start overflow-x-auto">
            ...
            <TabsTrigger value="reviews" className="data-[state=active]:bg-cosmic-orange data-[state=active]:text-black">Reviews ({product.review_count})</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="reviews" className="mt-8">
             <div className="flex flex-col items-center justify-center py-12 text-center bg-white/5 rounded-2xl">
                ...
                <p className="text-white/50 max-w-xs mx-auto">Be the first to share your experience with the {product.name}.</p>
